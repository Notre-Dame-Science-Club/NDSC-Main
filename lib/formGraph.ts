...
export type FormNodeAppearance = {
  ...
  // Percentage width (10-100) the cover image renders at, centered within
  // the form card. Defaults to 100 (full-bleed, edge to edge — the
  // previous, only, behavior) when unset.
  ...
  // Explicitly hides the children-picker heading entirely — distinct from
  // leaving children_heading blank, which falls back to the hardcoded
  // default text above. Without this there was no way to show nothing.
  hide_children_heading?: boolean
  // When set, the runner ignores this node's own title/subtitle/
  // cover_photo_url and shows the parent event/olympiad's own title,
  // description, and cover image instead — so the starter form's banner
  // always matches the event page without the admin copy-pasting it in
  // twice. Mirrors the legacy form_configs auto_pull_title/description/
  // cover columns (see app/admin/forms/page.tsx) that this per-node
  // appearance shape didn't carry over when it replaced that system.
...
export type FormNodeBehavior = {
  ...
  require_team?: {
    ...
    // inputs (full name/phone/email/college roll) — same FieldValidation
    // shape as any other field (lib/formBlocks.ts). When set for
    // college_roll specifically, fully replaces the NDC-conditional
    // 8-digit default in lib/validation.ts (including disabling it).
    member_field_validation?: {
      full_name?: import('./formBlocks').FieldValidation
      phone?: import('./formBlocks').FieldValidation
      email?: import('./formBlocks').FieldValidation
      college_roll?: import('./formBlocks').FieldValidation
    }
  }
  requires_payment?: {
    amount: number
