...
/**
 * Every successful API response passes through here, so this is the one place
 * that can guarantee no broken upload URL ever reaches the browser.
 *
 * Hostinger's upload.php sometimes reports a file as
 * `…/uploads/covers/x.jpg` when it actually lives at `…/covers/x.jpg`, and
 * older rows still carry the `ndscbd.net/uploads/…` and `arnob.…` forms.
 * `normalizeUploadUrlsDeep` walks the payload and rewrites only strings that
 * are recognisably upload URLs — everything else is returned untouched.
 *
 * This is a display-time safety net, not a replacement for fixing the stored
 * data: uploads are normalized on write too, and /api/admin/fix-urls repairs
 * existing rows.
 */
