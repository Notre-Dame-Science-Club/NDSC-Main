...
export type AudienceMember = {
  id: string
  batch?: string | null
  stage?: 'member' | 'organizer' | 'executive' | 'alumni' | null
} | null
...
export function matchesAudience(
  ...
  switch (audienceType) {
    ...
    case 'members':
      return !!member
    case 'organizers':
      return member?.stage === 'organizer'
    case 'executives':
      return member?.stage === 'executive'
    case 'alumni': {
      if (!member) return false
      // Alumni can be identified by stage='alumni' OR by batch filtering
      if (member.stage === 'alumni') return true
      if (!member.batch) return false
    ...
    case 'custom': {
      ...
      if (roles.includes('organizer') && member.stage === 'organizer') return true
      if (roles.includes('executive') && member.stage === 'executive') return true
