...
export type FormBlock = {
  ...
  // Generic format-validation rule, usable on ANY text-ish field —
  // built-in (phone, college_roll, ...) or custom. Previously "college roll
  // must be exactly 8 digits" was hardcoded server-side and only applied to
  // that one field; this replaces that with a config any field can turn on
  // (or explicitly off) for itself. See validateFieldFormat() below.
  validation?: FieldValidation
  // content-only: percentage width (10-100) for the 'image' content block.
  // Defaults to 100 (full width of the form) when unset. Always centered.
  width_pct?: number
}

export type FieldValidation = {
  // The toggle the admin flips to turn this rule on/off without losing the
  // numbers they'd already configured.
  enabled?: boolean
  digits_only?: boolean
  exact_length?: number | null
  min_length?: number | null
  max_length?: number | null
  // Overrides the auto-generated message below when set.
  message?: string
}

// Checks a single field's typed value against its own `validation` config.
// Isomorphic — used by the public FieldsRenderer (live, on blur) and the
// form-graph submit route (authoritative, server-side). Returns null when
// the value is fine (including when validation is off, or the field is
// empty — required-ness is a separate, existing check).
export function validateFieldFormat(field: Pick<FormBlock, 'label' | 'validation'>, rawValue: any): string | null {
  const v = field.validation
  if (!v || !v.enabled) return null
  if (Array.isArray(rawValue)) return null // multi-select / file fields don't use this
  const value = (rawValue ?? '').toString().trim()
  if (!value) return null
  const name = field.label || 'This field'
  if (v.digits_only && !/^\d+$/.test(value)) {
    return v.message || `${name} must contain digits only.`
  }
  if (v.exact_length != null && value.length !== v.exact_length) {
    return v.message || `${name} must be exactly ${v.exact_length} character${v.exact_length === 1 ? '' : 's'} long.`
  }
  if (v.min_length != null && value.length < v.min_length) {
    return v.message || `${name} must be at least ${v.min_length} character${v.min_length === 1 ? '' : 's'} long.`
  }
  if (v.max_length != null && value.length > v.max_length) {
    return v.message || `${name} must be at most ${v.max_length} character${v.max_length === 1 ? '' : 's'} long.`
  }
  return null
}
...
// here means adding a matching column on activity_registrations (or, for
// 'batch', on olympiad_registrations — see below).
//
// 'batch' is olympiad-only: activity_registrations has no `batch` column
// (only olympiad_registrations and members do), so builtinFieldDefs() below
// only includes it for owner_kind 'olympiad', and the submit route strips it
// before writing to activity_registrations. Everything else applies to both.
export type BuiltinFieldKey =
  | 'full_name' | 'phone' | 'email' | 'college'
  | 'college_roll' | 'hsc_session' | 'division' | 'batch'

// The default fields pre-loaded into every new segment's form_field_schema.
...
//
// `ownerKind`: pass 'olympiad' to also seed the 'batch' field — it only has
// a matching column on olympiad_registrations, so activity graphs (the
// default, ownerKind omitted) don't get it.
export function builtinFieldDefs(ownerKind?: 'olympiad' | 'activity'): FormBlock[] {
  const defs: FormBlock[] = [
  ...
  if (ownerKind === 'olympiad') {
    defs.push({ id: 'batch', kind: 'field', type: 'text', label: 'Batch', description: '', required: false, placeholder: 'e.g. 2024', is_builtin: 'batch', db_column: 'top_level' })
  }
  return defs
}
