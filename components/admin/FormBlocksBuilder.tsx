...
import {
  ...
  blankBlock, isFieldBlockType, type FieldValidation,
} from '@/lib/formBlocks'
  ...
  return (
    <div className="space-y-4">
      {/* Palette. Content blocks (header/paragraph/image/etc.) only make
          sense on a full page-form, not inside a per-item fields list like
          "info collected per team member" — that context has nowhere to
          render them, and the parent's onChange there silently drops any
          non-field block, which used to make it look like nothing happened
          (or that adding a field had "failed") when someone clicked one.
          fieldsOnly hides that palette so there's nothing to click that
          would vanish without explanation. */}
      <div className={fieldsOnly ? '' : 'grid sm:grid-cols-2 gap-3'}>
        {!fieldsOnly && (
          <div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {(fieldsOnly ? FIELD_BLOCK_TYPES.filter(t => t.type !== 'mcq' && t.type !== 'checkbox' && t.type !== 'short_answer') : FIELD_BLOCK_TYPES).map(t => {
  ...
  if (block.kind === 'field') {
    return (
      <>
        ...
        {(block.type === 'text' || block.type === 'number' || block.type === 'textarea' || block.type === 'short_answer') && (
          <FieldValidationEditor value={block.validation} onChange={v => onPatch({ validation: v })} />
        )}
      </>
    )
  }
    ...
    case 'image':
      return (
        <>
          ...
          <div className="flex items-center gap-2">
            <label className="text-xs whitespace-nowrap" style={{ color: 'var(--muted)' }}>Width</label>
            <input type="range" min={10} max={100} step={5} value={block.width_pct ?? 100}
              onChange={e => onPatch({ width_pct: Number(e.target.value) })} className="flex-1" />
            <span className="text-xs w-10 text-right" style={{ color: 'var(--muted)' }}>{block.width_pct ?? 100}%</span>
          </div>
          {block.image_url && (
            <div className="w-full flex justify-center">
              <img src={block.image_url} alt="" className="mt-1 rounded-lg object-cover"
                style={{ width: `${block.width_pct ?? 100}%`, maxHeight: 160 }} />
            </div>
          )}
        </>
      )
...
// Format-validation editor, shown under any free-typed field (text, number,
// long text, short answer) — and reused for the four fixed per-team-member
// identity fields (full name/phone/email/college roll) in the node editor's
// Behavior panel, since those aren't FormBlocks themselves. Kept off by
// default (`enabled` toggle) so it never surprises anyone who doesn't touch
// it — the exact complaint that motivated this: a fixed "college roll must
// be 8 digits" rule used to be hardcoded server-side with no way to see,
// change, or turn it off. Now any field — phone number included — can have
// its own rule, and it's plainly visible and optional.
export function FieldValidationEditor({ value, onChange }: { value?: FieldValidation; onChange: (v: FieldValidation) => void }) {
  const v = value || {}
  const patch = (p: Partial<FieldValidation>) => onChange({ ...v, ...p })
  return (
    <div className="rounded-lg p-2.5 space-y-2" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border)' }}>
      <label className="flex items-center gap-2 text-xs font-semibold cursor-pointer" style={{ color: v.enabled ? 'var(--white)' : 'var(--muted)' }}>
        <input type="checkbox" checked={!!v.enabled} onChange={e => patch({ enabled: e.target.checked })} />
        Restrict what can be typed here
      </label>
      {v.enabled && (
        <div className="space-y-2 pl-1">
          <label className="flex items-center gap-2 text-xs" style={{ color: 'var(--muted)' }}>
            <input type="checkbox" checked={!!v.digits_only} onChange={e => patch({ digits_only: e.target.checked })} />
            Digits only
          </label>
          <div className="grid grid-cols-3 gap-2">
            <div>
              <label className="block text-[10px] mb-1" style={{ color: 'var(--border-soft)' }}>Exact length</label>
              <input type="number" min={0} placeholder="—" value={v.exact_length ?? ''}
                onChange={e => patch({ exact_length: e.target.value ? Number(e.target.value) : null })}
                className={inputCls} style={inputStyle} />
            </div>
            <div>
              <label className="block text-[10px] mb-1" style={{ color: 'var(--border-soft)' }}>Min length</label>
              <input type="number" min={0} placeholder="—" value={v.min_length ?? ''}
                onChange={e => patch({ min_length: e.target.value ? Number(e.target.value) : null })}
                className={inputCls} style={inputStyle} />
            </div>
            <div>
              <label className="block text-[10px] mb-1" style={{ color: 'var(--border-soft)' }}>Max length</label>
              <input type="number" min={0} placeholder="—" value={v.max_length ?? ''}
                onChange={e => patch({ max_length: e.target.value ? Number(e.target.value) : null })}
                className={inputCls} style={inputStyle} />
            </div>
          </div>
          <input placeholder='Custom error message (optional, e.g. "Roll must be 8 digits")'
            value={v.message || ''} onChange={e => patch({ message: e.target.value })}
            className={inputCls} style={inputStyle} />
        </div>
      )}
    </div>
