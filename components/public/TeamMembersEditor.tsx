...
import { validateFieldFormat, type FieldValidation } from '@/lib/formBlocks'
...
export type TeamFieldDef = {
  ...
  options?: string[]
  /** Format-validation rule for this extra field — same mechanism as any
   *  other field (lib/formBlocks.ts). */
...
export type TeamMembersEditorProps = {
  ...
  config: {
    ...
    fields?: TeamFieldDef[]
    /** Format-validation overrides for the 4 fixed identity inputs
     *  (full name/phone/email/college roll). Same as
     *  behavior.require_team.member_field_validation server-side. */
  ...
  /** Fires whenever whether every visible member row currently passes
   *  its format-validation rules changes, so the parent (FormRunner) can
   *  hold off submitting instead of round-tripping to the server. */
  onValidityChange?: (allValid: boolean) => void
  /** Parent sets this after a blocked submit attempt so every violation
   *  shows up immediately, not just the ones already blurred past. */
...
// Small inline error line shown right under an offending field — mirrors
// FieldsRenderer's FormatErrorNote so identity/extra fields on a team
// member row and fields on the main form look and behave the same way.
...
function ExtraFieldInput({
  field, value, onChange, onBlur, formatError, accent,
}: {
  field: TeamFieldDef
  value: any
  onChange: (v: any) => void
    ...
    return (
      <div>
        ...
        <textarea value={value || ''} onChange={e => onChange(e.target.value)}
          onBlur={onBlur}
          rows={3}
          className={inputCls} style={formatError ? { ...inputStyle, borderColor: 'var(--danger-soft)' } : inputStyle} />
...
export default function TeamMembersEditor({ value, onChange, accent, config, teamName, onTeamNameChange, onValidityChange, forceShowErrors }: TeamMembersEditorProps) {
  ...
  // Format-validation errors for every row — the 4 fixed identity fields
  // plus any extra fields with their own `validation` config. Keyed by
  // member id so each row's inputs can look up just their own.
  const formatErrors: Record<string, Record<string, string>> = {}
  for (const m of value) {
    const rowErrors: Record<string, string> = {}
    const fixed: Array<[string, string, FieldValidation | undefined]> = [
      ['full_name', m.full_name || '', mfv.full_name],
      ['phone', m.phone || '', mfv.phone],
      ['email', m.email || '', mfv.email],
      ['college_roll', m.college_roll || '', mfv.college_roll],
    ]
    for (const [key, val, rule] of fixed) {
      const err = validateFieldFormat({ label: key.replace('_', ' '), validation: rule }, val)
      if (err) rowErrors[key] = err
    }
    for (const field of fields) {
      if (!field.key) continue
      const err = validateFieldFormat(field, m.custom_answers?.[field.key])
      if (err) rowErrors[field.key] = err
    }
    if (Object.keys(rowErrors).length) formatErrors[m.id] = rowErrors
  }
  useEffect(() => {
    ...
    <div className="space-y-3 pt-2">
      ...
      <div className="space-y-3">
        {value.map((m, idx) => {
          ...
          const rowErrors = formatErrors[m.id] || {}
          const showErr = (key: string) => (forceShowErrors || rowTouched.has(key)) ? rowErrors[key] : undefined
          return (
            <div key={m.id}
              ...
              <div>
                <input placeholder="Full name" value={m.full_name}
                  ...
                  className={inputCls} style={showErr('full_name') ? { ...inputStyle, borderColor: 'var(--danger-soft)' } : inputStyle} />
                <FormatErrorNote message={showErr('full_name')} />
              </div>
              <div>
                <input placeholder="Phone" type="tel" value={m.phone || ''}
                  onChange={(e) => setRow(idx, { phone: e.target.value })}
                  onBlur={() => markTouched(m.id, 'phone')}
                  className={inputCls} style={showErr('phone') ? { ...inputStyle, borderColor: 'var(--danger-soft)' } : inputStyle} />
                <FormatErrorNote message={showErr('phone')} />
              </div>
              <div>
                <input placeholder="Email" type="email" value={m.email}
                  onChange={(e) => setRow(idx, { email: e.target.value })}
                  onBlur={() => markTouched(m.id, 'email')}
                  className={inputCls} style={showErr('email') ? { ...inputStyle, borderColor: 'var(--danger-soft)' } : inputStyle} />
                <FormatErrorNote message={showErr('email')} />
              </div>
              <div>
                <input placeholder="College roll" value={m.college_roll}
                  ...
                  className={inputCls} style={showErr('college_roll') ? { ...inputStyle, borderColor: 'var(--danger-soft)' } : inputStyle} />
                <FormatErrorNote message={showErr('college_roll')} />
              </div>
              {passwordRequired && (
              ...
              {fields.map((field) => {
                ...
                return (
                  <ExtraFieldInput key={k}
                    ...
                    onBlur={() => markTouched(m.id, k)}
                    formatError={showErr(k)}
                    accent={accent}
                  />
                )
              })}
