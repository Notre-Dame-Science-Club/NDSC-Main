 ...
 * page (not just the home page) gets the same `.reveal` / `.reveal-mask` /
 * `.reveal-stagger` scroll-in motion. (`.reveal-stagger` was missing from
 * the selector for a while — any section using only that class, e.g. the
 * About page's departments grid, never got `.visible` added and stayed at
 * `opacity: 0` forever.)
 *
...
export default function ScrollReveal() {
  useEffect(() => {
    ...
    const observe = () => {
      document
        .querySelectorAll(".reveal:not(.visible), .reveal-mask:not(.visible), .reveal-stagger:not(.visible)")
    ...
    if (reduced) {
      document
        .querySelectorAll(".reveal, .reveal-mask, .reveal-stagger")
