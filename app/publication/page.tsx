    ...
    <div className="min-h-screen" style={{ paddingTop: '72px', background: 'var(--bg)' }}>
      ...
      {pdfOpen && selectedPub && (
        <div className="fixed inset-0 z-50 flex flex-col" style={{ background: 'rgba(0,0,0,0.97)' }}>
          ...
          <PdfViewer url={selectedPub.pdf_url} flipbookUrl={selectedPub.flipbook_url} />
        </div>
      )}
            ...
            <div className="flex justify-center lg:justify-end">
              {audri ? (
                <Book3D
                  ...
                  title={audri.title}
                  onClick={() => (audri.pdf_url || audri.flipbook_url) && openPdf(audri)}
                />
              ) : (
              ...
              {audri ? (
                <div className="flex flex-col gap-6">
                  ...
                  <div className="flex flex-wrap gap-4 mt-4">
                    {/* READ ONLINE used to point at one hardcoded Heyzine link,
                        so it always opened the 2025 issue. Now it opens this
                        issue's own flip-book, or its PDF if none is set. */}
                    {(audri.flipbook_url || audri.pdf_url) && (
                      <button
                        onClick={() => openPdf(audri)}
    ...
    <>
      {open && (
        <div className="fixed inset-0 z-50 flex flex-col" style={{ background: 'rgba(0,0,0,0.97)' }}>
          ...
          {(pub.pdf_url || pub.flipbook_url) && (
            <PdfViewer url={pub.pdf_url} flipbookUrl={pub.flipbook_url} />
          )}
        </div>
      )}

      <div className="reveal group cursor-pointer" style={{ width: 120 }}
        onClick={() => (pub.pdf_url || pub.flipbook_url) && setOpen(true)}>
