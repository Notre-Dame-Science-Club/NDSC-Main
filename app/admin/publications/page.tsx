  ...
  return new Promise(async (resolve) => {
    ...
    xhr.addEventListener('load', () => {
      try {
        ...
        // Hostinger's upload.php sometimes returns the URL with an extra
        // /uploads/ segment (…/uploads/covers/x.jpg) even though the file
        // actually lands at …/covers/x.jpg. Saving that raw value straight
        // into the DB was the cause of every broken publication cover.
...
const handlePdf = async (e: ChangeEvent<HTMLInputElement>) => {
  ...
  const removeCategory = async (value: string) => {
    ...
    const next = categories.filter((cat) => cat.value !== value)
    persistCategories(next)
    if (form.category === value) {
      setForm((prev) => ({ ...prev, category: next[0]?.value || DEFAULT_CATEGORIES[0].value }))
    }
    try {
      const res = await fetch('/api/admin/publications/categories', {
        method: 'DELETE',
      ...
      setMsg('Could not remove category — it may reappear on reload if a publication still uses it')
      setMsgOk(false)
    }
  }
  ...
  const edit = (item: Publication) => {
    ...
    setForm({
      ...
      flipbook_url: item.flipbook_url || '',
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <div className="mt-2 flex flex-wrap gap-1.5">
              {categories.map((c) => (
                <span key={c.value}
                  className="inline-flex items-center gap-1 pl-2 pr-1 py-1 rounded-full text-[11px]"
          ...
          <div className="md:col-span-2">
            <label className={lbl} style={{ color: 'var(--muted)' }}>Flip-book URL (optional)</label>
            <input className={inp} style={s}
              value={form.flipbook_url}
              onChange={e => setForm({ ...form, flipbook_url: e.target.value })}
              placeholder="https://heyzine.com/flip-book/xxxx.html" />
            <p className="text-[11px] mt-1" style={{ color: 'var(--muted)' }}>
              এই issue এর নিজের flip-book link. খালি রাখলে uploaded PDF টাই viewer এ দেখাবে।
            </p>
          </div>
