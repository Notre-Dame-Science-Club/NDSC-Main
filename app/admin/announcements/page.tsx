  ...
  return (
    <div>
      ...
      <div className="rounded-xl border p-6 mb-6" style={s}>
        ...
        <div className="mb-3">
          <label className="block text-xs mb-1" style={{ color: '#6a8faf' }}>Title (optional)</label>
        ...
        <div className="mb-3">
          <label className="block text-xs mb-1" style={{ color: '#6a8faf' }}>Message</label>
        ...
        <div className="mb-4">
          <label className="block text-xs mb-2" style={{ color: '#6a8faf' }}>Send to</label>
            ...
            {(['all', 'members', 'non_members'] as const).map(t => {
              ...
              return (
                <button key={t} onClick={() => setTarget(t)}
                  ...
                  style={{
                    borderColor: target === t ? '#00d4ff' : '#0f2a4a',
                    color: target === t ? '#00d4ff' : '#6a8faf',
                    background: target === t ? 'rgba(0,212,255,0.1)' : 'transparent',
                  }}>
        ...
        {result && (
          <div className="mb-4 p-3 rounded-lg text-sm" style={{
            background: result.ok ? 'rgba(0,255,128,0.1)' : 'rgba(255,80,80,0.1)',
            border: `1px solid ${result.ok ? 'rgba(0,255,128,0.3)' : 'rgba(255,80,80,0.3)'}`,
            color: result.ok ? '#00ff80' : '#ff7070',
          }}>
            {result.text}
          </div>
        )}
      ...
      <div className="rounded-xl border" style={s}>
        <div className="p-5 border-b" style={{ borderColor: '#0f2a4a' }}>
          <h2 className="font-bold text-sm" style={{ color: '#00d4ff', fontFamily: "'Orbitron', sans-serif" }}>
            Past Announcements
          </h2>
        </div>
        {loading ? (
          <p className="p-6 text-sm" style={{ color: '#6a8faf' }}>Loading...</p>
        ) : history.length === 0 ? (
          <p className="p-6 text-sm" style={{ color: '#6a8faf' }}>No announcements sent yet.</p>
        ) : (
          <div className="divide-y" style={{ borderColor: '#0a1f35' }}>
            {history.map(a => {
              ...
              return (
                <div key={a.id} className="p-4">
                  <div className="flex items-center justify-between mb-1 flex-wrap gap-2">
                    <p className="font-bold text-sm" style={{ color: '#e8f4ff' }}>{a.title}</p>
                    <span className="flex items-center gap-1 text-xs px-2 py-0.5 rounded-full"
                      style={{ background: 'rgba(0,212,255,0.08)', color: '#6a8faf' }}>
                      <Icon size={12} /> {TARGET_LABEL[safeTarget]}
                    </span>
                  </div>
                  <p className="text-sm" style={{ color: '#6a8faf' }}>{a.body}</p>
