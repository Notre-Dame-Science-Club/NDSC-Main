...
type Member = {
  ...
  stage: 'member' | 'organizer' | 'executive' | 'alumni'
...
export default function AdminMembersPage() {
  ...
  // Updates member stage — used to build the "Organizers" / "Executives" /
  // "Alumni" audience options in the Surveys admin page (see lib/survey.ts).
  const updateStage = async (m: Member, stage: 'member' | 'organizer' | 'executive' | 'alumni') => {
    setError('')
    try {
      const res = await fetch('/api/admin/members', {
        method: 'PUT', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: m.id, stage }),
      })
      if (!res.ok) { const d = await res.json(); setError(d.error || 'Could not update member.'); return }
      setMembers(prev => prev.map(x => x.id === m.id ? { ...x, stage } : x))
    } catch { setError('Network error.') }
  }
        ...
        <table className="w-full text-sm">
          <thead>
            <tr style={{ borderBottom: '1px solid var(--border)', background: 'rgba(var(--blue-rgb), 0.05)' }}>
              ...
              <th className="text-left px-4 py-3 font-medium" style={{ color: 'var(--muted)' }}>Stage</th>
              ...
              return (
                <tr key={member.id} style={{ borderBottom: '1px solid var(--surface-alt)' }}>
                  ...
                  <td className="px-4 py-3">
                    <select
                      value={member.stage}
                      onChange={e => updateStage(member, e.target.value as 'member' | 'organizer' | 'executive' | 'alumni')}
                      title="Member stage — used for survey/notification audience targeting"
                      className="px-2 py-1 rounded text-xs border outline-none"
                      style={{
