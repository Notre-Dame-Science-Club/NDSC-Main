...
type ActivitySession = {
  ...
  event_start: string | null; event_end: string | null; // legacy range fields, kept for old rows
  session_date: string; location: string; // session_date kept for backward compatibility
      ...
      <div className="p-5 flex-1 flex flex-col">
        ...
        <div className="mt-auto flex items-end justify-between gap-3 text-xs" style={{ color: "var(--muted)" }}>
          <div className="flex flex-col gap-1 min-w-0">
            {(s.event_dates?.length || s.event_start || s.session_date) && (
              <span className="inline-flex items-center gap-1 truncate">
                <CalendarDays size={12} />
                {s.event_dates && s.event_dates.length > 0 ? (
                  s.event_dates.length > 1
                    ? `${new Date(s.event_dates[0]).toLocaleDateString("en-BD", { day: "numeric", month: "short" })} – ${new Date(s.event_dates[s.event_dates.length - 1]).toLocaleDateString("en-BD", { day: "numeric", month: "short", year: "numeric" })} (${s.event_dates.length}-day event)`
                    : new Date(s.event_dates[0]).toLocaleDateString("en-BD", { day: "numeric", month: "short", year: "numeric" })
                ) : s.event_start ? (
                  s.event_end && s.event_end !== s.event_start
                    ? `${new Date(s.event_start).toLocaleDateString("en-BD", { day: "numeric", month: "short" })} – ${new Date(s.event_end).toLocaleDateString("en-BD", { day: "numeric", month: "short", year: "numeric" })}`
                    : new Date(s.event_start).toLocaleDateString("en-BD", { day: "numeric", month: "short", year: "numeric" })
                ) : (
                  new Date(s.session_date).toLocaleDateString("en-BD", { day: "numeric", month: "short", year: "numeric" })
                )}
              </span>
            )}
