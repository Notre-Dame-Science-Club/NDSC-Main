  ...
  if (alreadyRegistered) {
    return (
      <div className="rounded-2xl border p-6 mb-8 flex items-center justify-between gap-4 flex-wrap"
        ...
        {/* Two actions, not one: "already registered" used to be a dead
            end here — the only link out was the dashboard, with no way to
            get back into the form. That's right for a plain event (you
            genuinely can't register twice), but wrong for a segmented one,
            where a second, independent registration for another segment
