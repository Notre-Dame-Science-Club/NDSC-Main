...
export async function GET(req: NextRequest) {
  ...
  // Which node the person is currently filling in. The public runner
  // sends this as the last entry of the path it's walking (the active
  // node) — the same id that ends up as the last entry of
  // submitted_node_ids once that node is actually submitted. We match
  // against OTHER registrations' own last-submitted node rather than a
  // fixed path position, so this correctly scopes to "this exact terminal"
  // regardless of how many segment/subsegment levels sit above it. Without
  // this, someone who already finished one leaf got told "you're already
  // registered" while typing into a DIFFERENT leaf that merely shared an
  // ancestor segment with the one they'd already completed.
  const segmentNodeId = req.nextUrl.searchParams.get('segment_node_id') || null
  ...
  // Scope to the same terminal when we know one — see the comment above.
  // A registration that hasn't reached this exact node as its last
  // submitted step (whether it's still mid-tree, or finished at a
  // DIFFERENT leaf entirely) can't match, so it's excluded rather than
  // treated as a false match. When segmentNodeId is null (non-segmented
  // event, or caller didn't send it) every registration passes through
  // unfiltered, same as before.
  const regs = segmentNodeId
    ? (allRegs || []).filter((r: any) => Array.isArray(r.submitted_node_ids) && r.submitted_node_ids.length > 0 && r.submitted_node_ids[r.submitted_node_ids.length - 1] === segmentNodeId)
    : allRegs
