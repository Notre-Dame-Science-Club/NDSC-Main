  ...
  try {
    ...
    const { data: created, error: insertError } = await supabaseAdmin
      ...
      .insert({
        ...
        payment_slip_url,
        is_verified: false,
        // This flow only ever validates the roll as an NDC roll (see
        // validateCollegeRoll('Notre Dame College', ...) above) — it never
        // asked for an institution, so `institution` was left null for
        // every OAuth signup. That silently broke isNDCStudent() (and
        // anything built on it, e.g. the olympiad eligibility check and
        // club membership applications) for genuine NDC students who
        // signed up with Google: their account looked "not NDC" simply
        // because the field was empty. Since this route only ever creates
        // NDC members, set it explicitly here.
