...
export type MemberStage = 'member' | 'organizer' | 'executive' | 'alumni'
...
export interface MemberRow {
  ...
  /** Unified stage for member role: member (default), organizer, executive, or alumni. Used for survey/notification audience targeting — see lib/survey.ts */
  stage: MemberStage
...
export interface ActivitySessionRow {
  ...
  title: string
  slug: string
  event_start: ISODateString | null // Event start date
  event_end: ISODateString | null   // Event end date (if null or same as start, it's a single-day event)
  session_date: ISODateString       // Legacy, being phased out
  location: string
  description: string
  cover_image_url: string
  youtube_url: string
  ...
  event_dates?: string[]            // Legacy, being phased out
